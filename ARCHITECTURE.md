...
- **Testing as a first-class concern** — pure math module is decoupled for isolated unit testing without any I/O
...
flowchart TD
    ...
    subgraph ctrl["controllers/"]
        CC["clientsController.js\nJoi schema — registerClientSchema\nregisterClient handler:\n  → clientService.registerClient()\n  → 201 JSON"]
        RC["rateLimitController.js\nJoi schema — checkRateLimitSchema\ncheckRateLimit handler:\n  → clientService.getClientByClientId()\n  → rateLimitService.checkRateLimit()\n  → 200 or 429 JSON + Retry-After header"]
    end

    SRV --> APP --> cfg & mw & mdl & svc & ctrl
```
...
quadrantChart
    ...
    x-axis "Memory Efficiency" --> "High Memory"
    ...
    Fixed Window: [0.9, 0.2]
    Sliding Log: [0.15, 0.9]
    Leaky Bucket: [0.85, 0.1]
```
...
sequenceDiagram
    ...
    alt ✅ Allowed
        RC-->>U: 200 {allowed:true, remainingRequests:99, resetTime:"..."}
    else 🚫 Rate Limited
        RC->>RC: res.setHeader("Retry-After", String(retryAfter))
        RC-->>U: 429 {allowed:false, retryAfter:36, resetTime:"..."}
    end
```
...
sequenceDiagram
    ...
    CS->>CS: await bcrypt.hash(apiKey, 12)
...
erDiagram
    CLIENTS {
        ...
        String hashedApiKey "bcrypt cost-12"
...
**Path encoding rationale:** `base64url` is used instead of raw path strings:
- Avoids Redis key injection with special characters
- Produces fixed-length deterministic keys
- URL-safe (no `+` or `/` characters in key names)
...
|---|---|---|
| `/check` is a POST, not GET | POST | Carries a request body (clientId + path); GET with body is non-standard |
...
| apiKey not in response | Omitted | Security: the key should never be echoed back in any response |
...
flowchart TD
    ...
    RT -->|"/api/v1/clients"| A1["🔑 authInternal\nChecks x-internal-api-key header"]
...
|---|---|---|
| API key exposure | Irreversible hashing | bcrypt (cost 12) via `bcryptjs` |
| Duplicate API key bypass | Uniqueness fingerprint | SHA-256 → unique MongoDB index |
| Unauthorized client creation | Header gate | `x-internal-api-key` middleware |
...
| Credential hardcoding | Environment variables | All secrets via `process.env` |
...
flowchart TB
    ...
    subgraph GH["GitHub Actions — ci.yml"]
        ...
        subgraph JOB1["Job: build-and-test  🔧\nruns-on: ubuntu-latest"]
            B1["📥 actions/checkout@v4"] --> B2
            B2["🐳 docker compose build app test\n(multi-stage Dockerfile)"] --> B3
            B3["▶️ docker compose up -d mongo redis\n(wait for healthchecks)"] --> B4
...
| `test` | inherits `deps` | Full source + tests | CI `docker compose run test` |
...
**Size optimization:** The `runner` stage excludes all test code, dev dependencies, and build tooling, resulting in a minimal, secure production image.
...
| **Testability** | `tokenBucketMath.js` is I/O-free — unit testable without any infrastructure |
